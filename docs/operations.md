...
macOS local confirmation opening is implemented; Linux relies on `xdg-open`; Windows automatic confirmation opening is not implemented. These paths are not a platform support certificate. Production HTTPS deployment/testing, additional browsers, an official distributed Open-Science application's compatibility, Connector release publication and real OAuth still need evidence. The GitHub App client ID awaits the user. Do not use preview port 4190 as a catalog source: Node fetch rejects it under its standard bad-port policy; the verified local website used 4193.
