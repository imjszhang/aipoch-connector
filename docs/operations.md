...
Setup additionally accepts `--origin EXACT_ORIGIN`, `--loopback-http allow|deny`, `--port PORT`, `--catalog-url MANIFEST_URL` and `--github-client-id REGISTERED_CLIENT_ID`. Origin entries are exact, without a trailing slash/path; the exact production entry defaults to `https://aipoch.network`. Alpha.4 additionally permits canonical HTTP origins with hostname `localhost`, `127.0.0.1` or `[::1]` at any port (including default HTTP port 80). Other hostnames, LAN addresses, aliases and local HTTPS are not implicitly permitted. Browser/Node restricted-port rules still apply. Setup atomically saves configuration, then compares the active core’s data-directory identity, configuration digest, package/code version and Node version. A mismatch restarts only the authenticated core for that data directory and verifies the replacement. Its output reports `started`, `restarted` or `unchanged` with `configurationApplied: true` only after verification. A restart invalidates in-memory pairings/sessions/action views; durable inbox and operation records remain. A legacy core without verifiable identity returns `runtime_identity_unconfirmed`: inspect and explicitly stop that older core before running setup again. No PID-based termination or cross-directory takeover is performed. The Network adapter currently uses port 47821; changing only Connector's port is not website discovery.

## Local HTTP policy and upgrade

`allowLoopbackHttp` is a boolean configuration field. Missing means true for new and existing configurations; reading old configuration does not rewrite its origins or other fields. Explicit false persists through subsequent setup. Invalid types are rejected.

`aipoch-connector setup --loopback-http deny` turns off implicit local HTTP permission; `allow` enables it. Setup requires a running user-selected host as before. To keep only the original explicit list during an upgrade, stop the old core and set `"allowLoopbackHttp": false` in its owner configuration before starting alpha.4. This is a configuration-only change; schema remains 2.

The exact `origins` list remains additive: turning the policy off does not remove local origins explicitly added by the owner. Remove such entries too if the intent is to deny all local websites. The service does not learn or save random website ports. Changed effective policy restarts only the authenticated core, invalidating old sessions/pairings and retaining receipts/associations/operations. Missing and true have the same effective digest.

A permitted local website still requires human approval. Sessions bind the complete origin, so hostnames and ports cannot exchange pairing or session credentials. Local origins remain prohibited on browser requests to owner administration. CORS never grants wildcard permission or replaces authentication. `[::1]` permits an IPv6 website origin; Connector itself still listens at `127.0.0.1` and the browser endpoint remains `http://127.0.0.1:47821`.

Local origin permission does not change the catalog manifest or canonical public reference location. Use owner `--catalog-url` when a local preview needs its matching snapshot. References still use the production HTTPS public location; custom/local publication URLs are not silently accepted. See [Network integration](network-integration.md).
...
New alpha.3 acquisition records preserve the complete input under `identityVersion: acquire-v1`. Identity comparison excludes only source resolution time and repository-license observation time, preserves all other source fields and conditions, and normalizes object key ordering and destination paths. Replays still resolve GitHub first, but do not fetch the file again or write it; use operation queries when GitHub is unavailable. Legacy records retain their original input digest. If it cannot be matched exactly, `legacy_operation_unverifiable` requires inspection of the old record and destination; no missing input is reconstructed and no replacement operation is automatic.
...
The runtime includes refresh handling for expiring credentials when a valid refresh token and configured client ID are available. Expired/revoked/failed credentials require explicit local reconciliation or renewed authorization; no device authorization is initiated silently. Actual App registration, final user consent, fresh-process Keychain readback and authenticated identity/public-source reads passed as recorded in [the real authorization evidence](verification/github-authorization.md). Live refresh rotation, revocation and private-repository access were not exercised; their existing fixture coverage does not establish a live result.
...
Upgrade by installing a reviewed version/tarball after stopping the old core; rerun setup if the registered executable path changed, and verify status/tool discovery. Current schema version is 2. Alpha.3 adds acquisition input evidence in a transaction, preserving schema-1 receipts, associations and operations. Alpha.1/alpha.2 cannot open schema 2; a deliberate rollback needs the pre-upgrade backup and must account for newer records separately. A future database version is rejected rather than silently downgraded. Preserve the backup and use the compatible software version; there is no automatic destructive reset or reverse migration.
...
The Connector `v0.1.0-alpha.1` GitHub release is published from `98ad82b246805cdcdf27712c4ffbf05e8f25f44d`; hosted CI/release checks passed 101 tests, and the downloaded tarball matched its published checksum. Network initially deployed real mode from `e3eb65895b1bebe64f3cefcf18996213bf525313`, then deployed receipt recovery from `bf4b365f1fff865e60bd76a6d0de20ccf5ec64db` in successful Pages run `34761579849`. The repair passed 283 unit checks and the complete browser matrix; public bytes matched the reviewed artifact. See [the alpha release record](verification/alpha-release.md) and [production browser evidence](verification/production-browser.md).

Production Chrome pairing and the deployed resource-receipt retest passed against the rebuilt development host: one reviewed Send produced **Reference received**, one visible home receipt and matching receipt history/durable original-content digest. Same-session Disconnect restored public state without a reload, and an owner read/hash after disconnect confirmed receipt retention. The earlier unconfirmed project request was retained and not resent. Real authorization and authenticated public-source reads through the registered AIPOCH Connector App also passed. macOS local confirmation opening is implemented; Linux relies on `xdg-open`; Windows automatic confirmation opening is not implemented. This tested Chrome/macOS flow does not certify additional browsers, operating systems or an official distributed Open-Science application. Do not use preview port 4190 as a catalog source: Node fetch rejects it under its standard bad-port policy; the verified local website used 4193.
...
## Remembered browser authorization

See [Manage authorized browsers](../README.md#manage-authorized-browsers) for local product entry points and [the extension contract](persistent-authorization.md) for lifecycle and trust boundaries. `authorizations.sqlite` is separate from inbox schema 2 and contains only public-key grants and the Connector installation identity. Preserve it during normal upgrades/restarts to retain remembered authorization. Removing it forces new browser approval; never substitute a different installation identity into existing browser metadata. A rollback binary cannot restore remembered sessions, although its original explicit pairing remains available.

Use `authorizations list` for owner-visible origin/label/last-use/state and `authorizations manage` for the private per-browser revoke UI. `authorizations revoke ID` explicitly revokes the selected grant. When Network could not deliver Forget because Connector was offline, use this local entry after Connector starts. Revocation affects all sessions using that credential and leaves other browser credentials and inbox records intact. The pending grants and private management URLs are local account information, not public diagnostics.
