...
The Connector alpha is published from `98ad82b246805cdcdf27712c4ffbf05e8f25f44d`, with successful hosted 101-test checks and an independently verified release tarball. Network real mode deployed from `e3eb65895b1bebe64f3cefcf18996213bf525313` in successful Pages run `34758098215`; its reviewed artifact, deployment smoke and public HTTPS build-info/manifest checks passed. Publication identities and evidence are recorded in [the alpha release record](verification/alpha-release.md) and [verification progress](verification/progress.md). The SDK pin is a separate upstream dependency identity.

Production browser pairing and real authorization with the user's existing GitHub App remain pending. The production browser check must record the actual browser/operating system, website origin, Connector version and authenticated host build/profile, then verify pairing, retained selection, reviewed sending, matching durable receipt and disconnected state. A routed fixture, local HTTP session or public HTTPS GET does not establish this browser-to-loopback chain. Additional browsers, operating systems and official Open-Science installers require their own compatibility evidence before support is advertised.
