...
Actual acceptance additionally records the exact browser, operating system, website origin, Connector package/version and authenticated host build/profile: explicit project association/file acquisition, denial, expiry, disconnect, host restart and uncertain-delivery behavior. A routed browser fixture or local HTTP result is not proof that every browser permits production HTTPS-to-loopback access. Production Network mode, additional browsers, remaining action/recovery checks and packaged Open-Science support remain pending until supported by evidence. Final implementation SHAs must be filled after actual commits, not inferred from the SDK pin.
