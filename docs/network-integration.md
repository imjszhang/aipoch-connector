...
From alpha.4, HTTP previews on localhost, 127.0.0.1 and [::1] at any port need no per-port origin entry while the default local policy is enabled. Configure the matching catalog manifest in the Connector when the snapshot differs from production. For example, a Network preview on `http://127.0.0.1:4186` uses `setup --catalog-url http://127.0.0.1:4186/catalog/v1/manifest.json`, followed by a Connector restart if already running. The optional catalog URL is local-owner configuration; a website cannot choose arbitrary backend fetch targets. A preview built from older fixtures will correctly fail `catalog_changed` if the Connector still validates against today's live catalog. Fix the test configuration rather than bypassing snapshot validation.
...
## Local preview policy limits (alpha.4)

`setup --loopback-http deny` disables only the implicit local HTTP rule; explicit owner origins remain usable. Site hostname and port remain part of session identity. A Network preview uses production canonical HTTPS `public_location` fields even when served locally; changing only the bridge policy never permits arbitrary publication URLs. Do not derive the trusted catalog or review base from an incoming browser Origin. No frontend changes, Network deployment or browser preference migration are required by the Connector policy change.

Automated HTTP/browser-fixture results and real Network + user-approved host results must be recorded separately in [the alpha.4 evidence](verification/alpha4-loopback.md). Historical real local receipts above do not establish the new default policy on their own.
