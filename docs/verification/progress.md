...
## Alpha.4 default local HTTP policy — 2026-09-14

PR #4 implemented and merged the default HTTP loopback policy, explicit opt-out, legacy configuration compatibility and unchanged exact-origin authority boundaries. Version alpha.4 was published from `3ef23c6950b85f0557ed17f9d04d6f3efea1be06`; 118/118 local and hosted tests, independent installation and downloaded-original asset verification passed. Actual Chrome transport fixtures covered two localhost ports, IPv4 and IPv6. The full new real Network + user-approved Open-Science delivery chain was not rerun, and historical claims remain scoped. See [the complete evidence and limitations](alpha4-loopback.md).
