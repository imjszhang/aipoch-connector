...
async function main() {
  ...
  if(command==='setup') {
    ...
    const runtime=await ensureRuntime(dataDir);output({configured:true,registration,...runtime,message:runtime.action==='restarted'
      ? 'Connector restarted and the saved configuration is active. Pair website sessions again. Use a new Open-Science session if its tool list is already loaded.'
  ...
  if(command==='pair') {
    ...
    if(action==='approve'){const code=option('code'),origin=option('origin');if(!code||!origin)throw new Error('Compare the code and website before supplying --code and --origin.');output(await adminRequest(dataDir,`/admin/pairings/${encodeURIComponent(id)}/approve`,'POST',{code,origin}));return;}
