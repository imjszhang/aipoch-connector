...
GitHub tools resolve existing URLs, list fixed-version files, preview a selected regular file and its SHA-256, and acquire that file into a **new** destination after explicit approval. They do not clone or synchronize complete repositories, overwrite files, install packages or run research. Public reads work without GitHub authorization. Optional `github auth` commands provide local device authorization with macOS Keychain storage, but require your registered GitHub App client ID and separate real-authorization verification; no production App registration is embedded or claimed. See [operations](docs/operations.md) and [GitHub boundaries](docs/github.md).
