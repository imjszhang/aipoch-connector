...
try {
  const packed = JSON.parse((await run(npm, ['pack', '--ignore-scripts', '--json', '--pack-destination', temporary])).stdout);
  ...
  await run(npm, ['install', '--prefix', install, '--ignore-scripts', '--package-lock=false', '--no-audit', '--no-fund', join(temporary, packed[0].filename)]);
  ...
  process.stdout.write(`Packed and independently installed aipoch-connector ${installed.version}; CLI help, public SDK integrity and local inbox import passed. No runtime or host was started.\n`);
} finally {
